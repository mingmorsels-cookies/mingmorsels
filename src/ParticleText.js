/**
 * ParticleText.js
 * React Bits ParticleText Component Implementation.
 * Samples text glyphs into particles that gather into place with smooth easing,
 * idle drift, pointer repel physics, and hover/click scatter triggers.
 */
...
export function initParticleText(container, options = {}) {
  ...
  container.innerHTML = `
    <div class="particle-text ${className}" aria-label="${text}">
      <canvas class="particle-text__canvas" aria-hidden="true"></canvas>
      <span class="particle-text__sr">${text}</span>
    </div>
  `;

  const particleTextEl = container.querySelector('.particle-text');
  ...
  const sampleText = async () => {
    ...
    const rect = particleTextEl.getBoundingClientRect();
    ...
    const computed = window.getComputedStyle(particleTextEl);
    const resolvedFamily = fontFamily === 'inherit' ? computed.fontFamily || 'sans-serif' : fontFamily;
    let resolvedSize = resolveFontSize(fontSize, particleTextEl, fontWeight, resolvedFamily);
  ...
  resizeObserver.observe(particleTextEl);
  sampleText();
