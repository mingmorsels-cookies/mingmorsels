...
const handleCreateOrder = async (req, res) => {
  try {
    ...
    const { items, coupon_code, user_email, user_name, shipping_address, receipt } = parseResult.data;
...
router.post('/order/cancel', validateCustomerSession, handleCancelOrder);
router.post('/shipping/cancel', validateCustomerSession, handleCancelOrder);

/**
 * GST Tax Invoice Endpoint (Protected against IDOR)
 */
router.get('/orders/:id/invoice', validateCustomerSession, async (req, res) => {
  try {
    const orderId = req.params.id;
