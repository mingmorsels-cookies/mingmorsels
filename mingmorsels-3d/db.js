...
async function readLocalStoreAsync() {
  try {
    ...
    return { users: [], orders: [], abandonedCarts: [], reviews: [], adminNotifications: [], inventory: { ...DEFAULT_INVENTORY }, pincodes: [] };
  }
}
...
export async function createOrderRecord(orderData) {
  const { id, user_id, user_name, user_email, items, total_amount, discount_amount, applied_coupon, tax_gst, delivery_fee, payment_status, razorpay_order_id, shipping_address } = orderData;

  if (pgPool) {
    try {
      const res = await pgPool.query(
        `INSERT INTO orders (id, user_id, user_name, user_email, items_json, total_amount, payment_status, razorpay_order_id, shipping_address)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         RETURNING *`,
        [id, user_id || null, user_name, user_email, JSON.stringify(items), total_amount, payment_status || 'PENDING', razorpay_order_id || null, shipping_address || '']
      );
      return res.rows[0];
    } catch (e) {}
  }
